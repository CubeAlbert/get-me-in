...
### 决策 293 —— 合并多语言专项最终事实并删除已完成执行文档

**背景：** 决策 292 完成多语言专项 L6 收口时，按用户当时的明确要求保留 `docs/multilingual-support.md`。用户现重新审查文档整洁度，并授权在内容已由当前文档体系承接时删除该专项文件。删除前审计确认：README 已记录三项语言配置；`docs/design.md` 已记录架构与依赖边界；`docs/plan.md`、`docs/task.md`、`docs/current.md` 已记录范围、完成状态、验证证据、禁止项和 R9 门禁；决策 284～292 与 Git 历史保留逐切片白名单和执行过程。

**决定：**

- 删除已完成的 `docs/multilingual-support.md`，不在 `docs/` 保留历史副本。
- 移除核心文档中指向该文件的活跃链接和“继续保留”措辞；当前架构、配置、边界、验证结果与下一步继续由 README 和五份核心文档保存。
- 保留决策 284～292 原文，不回写决策 292 当时的保留决定；以本决策记录后续授权变化。
- 本次只清理文档，不修改 production、测试、依赖或数据；多语言专项完成仍不构成 R9 授权，四个 legacy 数据目录的保护边界不变。

**理由：**

- 专项文件的剩余独有内容是已完成的逐切片执行白名单和新会话启动清单，不再属于当前活跃状态；Git 与追加式决策记录足以保留历史。
- 删除活跃文档中的历史执行副本可避免“已完成但仍实施中”“已收口但不得删除”等互相冲突的状态，并保持 `docs/current.md` 为唯一阶段快照。
- 新增决策而非修改历史决策，能够同时保持历史真实性与当前文档整洁度。
