...
- **验收标准：**
  ...
  - `src/request.py` — ✅ `Request` dataclass + `RequestType`（`USER_INPUT` / `CONTINUE`）
  - `src/tools/registry.py` — ✅ `Tool` dataclass + `@tool` 装饰器 + `ToolRegistry` + `agent_key` + `"*"` sentinel + `get_for(..., main_key=)` 参数
  - `src/tools/switch_tools.py` — ✅ `switch_to_subagent` + `switch_to_mainagent`，均通过 UIBridge 直连 CLI 交互
  - `src/cli/app.py` — ✅ 双循环 + questionary + FINISH 分支 switch 检测 + `/exit_sub` + UIBridge 轮询
  - `src/cli/uibridge.py` — ✅ `UIBridge` 跨线程通信桥（`select`/`confirm` + `get_bridge()` 模块级注入）
  - `src/agents/base.py` — ✅ Agent Loop（单步 + `_pending_tool`/`_pending_switch`/`_pending_reject` 三标记 + `_round_counter` 安全阀）+ `_get_agent_key()` + `_execute_tool()` 检测 `__switch__`/`__reject__` sentinel
  - `src/agents/main_agent.py` — ✅ 14 占位符 + `_get_agent_key()` → `MAIN_AGENT_KEY` + `_get_sub_agents_list()` 覆盖
  - `src/agents/registry.py` — ✅ `AgentRegistry` 全局单例 + `SubAgentDescriptor` + `list_agents_prompt()` + Agent key 常量
  - `src/agents/job_search/` — ✅ `JobSearchAgent(BaseAgent)`：测试用子 Agent
  - `main.py` — ✅ 组装全链路（`MainAgent` + `JobSearchAgent` + `AgentRegistry` + `ToolRegistry` → `App`）
  - `AGENT_MAX_ROUNDS` 环境变量 — ✅
  - `uv add questionary` — ✅
  - 端到端验证：主 Agent dispatch → JobSearchAgent 接管 → switch_to_mainagent 退回 — ✅
  - InterviewAgent — 后置到里程碑 7
- **前置依赖：** 里程碑 1（提示词 + LLM）+ 里程碑 2（RAG）+ 里程碑 3（记忆 + config）

### 里程碑 5 —— 简历 Agent ✅ 已完成

- **预期产出：** 通用 Plan 基础设施 + 工作区工具集 + 简历 Agent + 记忆集成 + 会话状态管理 + Esc 中断
- **验收标准：**
  - ✅ Plan 机制就绪（`create_plan` / `update_plan_status` / `cancel_all_plans` / `replan`）
  ...
  - ✅ 会话状态管理（auto-save / `/restore` / `/rewind` / `SaveManager`）
  - ✅ Esc 中断 Agent 处理（基础完成，即时中止 📌 暂缓）
  - ✅ 记忆集成（`AUTO_MEMORY_ON_EXIT` + `/build-memory` + `query_memory` 覆盖检索）
  - 📌 三路径输入 → 暂缓：改为 LLM 用 workspace 工具直接编辑 LaTeX
  - ⛔ 简历版本写入 + 历史检索 → 不予实现：`query_memory` 已覆盖
- **前置依赖：** 里程碑 4
