...
311. **修复 B00 malformed usage、scope 与 restore 覆盖并同步实施状态** — 非法 scalar／list／container usage details 归一化为 `MALFORMED`，`LLMAttemptRecord` 构造边界拒绝非法 scope，补齐费用同单位保留与换单位重算测试；活跃 design／plan／tracker 已改为工程实现完成、等待真实验收，完整测试增至 390 项。
