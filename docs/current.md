...
**当前子任务：** `docs/task.md` R8-G 5.2：归一化 `.env.example` 与 README，并完成 Settings 逐项核对。

**当前阻塞：** 无。R8-G 仅限 8 个文档／示例配置文件；若 G8 发现代码、测试、依赖或公开协议缺陷，必须停止并另行提交修复清单；R9 仍未授权。
...
**下一步：** 完成 R8-G 5.2：删除 `.env.example` 观察期／legacy rollback only 说明，逐项核对 `Settings.from_env()`，并将 README 改为当前 v2 事实；验证后独立 checkpoint 提交。
