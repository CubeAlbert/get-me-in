...
**当前子任务：** R8-F 3：更新 `SessionSnapshotCodec` 写入 `format_repairs_used` 与旧 `repair_attempted` 投影，并兼容旧快照读取。

**当前阻塞：** 无。OutputFormat／Parser 定向测试 19/19、Runtime 定向测试 26/26 已通过；真实模型稳定性须在全部代码 checkpoint 后由用户执行 smoke 验收。R9 未授权。
...
**下一步：** 让 snapshot codec 双写 `format_repairs_used` 与投影的 `repair_attempted`，恢复时优先严格读取非负整数计数，缺失时把旧 bool 映射为 0／1，并覆盖非法类型回归。不得检查、设计或实施 R9。
