...
**当前阶段：** 当前基线多语言 UI／模型回复语言专项 L3 已完成并提交，L4 尚未开始；R9 仍未授权

**当前任务：** 按 [`docs/multilingual-support.md`](multilingual-support.md) 继续多语言专项；L3 typed 固定事件文案与审批已完成，下一步实施 L4 ResponseLanguage Prompt 注入

**当前子任务：** L3 typed 固定事件文案与审批已完成：ProgressKind、canonical tool_name、CLI stable-key 投影；代码／测试 checkpoint 为 `cfca149`。
...
**下一步：** 按 L4 白名单新增 `06_response_language.md`，更新 PromptRenderer／bootstrap 及必要测试 fixture；保持 InputFormat、OutputFormat、ModelMessageCodec 和 repair 契约不变，不得进入 R9。
