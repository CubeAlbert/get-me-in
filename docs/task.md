...
- 🔄 L5：定向验证 `151/151`、完整 unittest `340/340`、compileall、diff-check、catalog／Prompt 静态 contract 和 fake/headless component smoke 已通过；双语言真实 provider／Windows TTY 矩阵待用户执行，其中英文 Memory smoke 必须由真实模型按既有被动契约调用一次 `query_memory`，不触发 Memory build。
