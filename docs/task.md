...
- 🔄 L5：定向验证 `151/151`、完整 unittest `341/341`、compileall、diff-check、catalog／Prompt 静态 contract 和 fake/headless component smoke 已通过；已补充语言配置中文默认行为、`.env.example` 选项注释，并将 general_agent Prompt 编号重排为 `01`～`10`，代码／测试提交为 `a5efaa1`、`c4ac8dd`；双语言真实 provider／Windows TTY 矩阵待用户执行，其中英文 Memory smoke 必须由真实模型按既有被动契约调用一次 `query_memory`，不触发 Memory build。
