...
- ✅ L4：新增 `06_response_language.md` 与 `RESPONSE_LANGUAGE` 注入；Main／Resume 使用相同 resolved locale，InputFormat／OutputFormat、ModelMessageCodec 和 format repair 不变。代码／测试 checkpoint 为 `8dc56a5`；完整 unittest `340/340`、compileall、diff-check、Prompt 静态 contract 和 L4 精确白名单审查通过。
- 🔄 L5：定向验证 `151/151`、完整 unittest `341/341`、compileall、diff-check、catalog／Prompt 静态 contract 和 fake/headless component smoke 已通过；已补充 `UI_LOCALE`、`MODEL_RESPONSE_LANGUAGE`、`LOCALES_DIR` 的中文默认行为和 `.env.example` 选项注释，代码／测试提交为 `a5efaa1`；双语言真实 provider／Windows TTY 矩阵待用户执行，其中英文 Memory smoke 必须由真实模型按既有被动契约调用一次 `query_memory`，不触发 Memory build。
- ⬜ L6：用户审查、README 与五份核心文档完成态、完成决策和独立文档 checkpoint；完成后迁移仍有效事实并删除专项文件，历史由 Git 与 decision 保存。
