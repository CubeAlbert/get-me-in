...
本文件本身不构成整体 Interviewer 实现授权。B00 小节已经包含该专项可直接实施的最终 API／类型／序列化清单、精确文件白名单和验收矩阵；决策 307 记录了 B00 coding 授权，决策 308 记录了工程实现 checkpoint。其余 Interviewer 阻塞点仍须在全部关闭后形成整体最终 API 清单、实施切片和验收矩阵，再由用户单独授权 coding。
...
| B00 | 逐次 LLM token usage 的采集、归属、持久化与查询 | 工程实现完成，等待真实验收 | B-01、当前 LLM／Session 链路 | 用户运行 Main／Resume provider smoke 与 estimator calibration |
...
B-01（已验证关闭）
  ↓
B00（工程实现完成；等待真实 provider／估算校准验收）
...
- B-01 已验证关闭；本项方案、精确 API／类型／序列化清单、文件白名单与验收边界均已确认。B00 工程实现与两轮代码复审已按白名单完成，当前仅保留真实 provider／估算校准验收门禁。

**当前代码事实**

- `LLMResult` 已返回可选 `content`、实际 response model 与 provider-neutral usage；`OpenAILLMAdapter` 读取 Chat Completions 的 input／output、可选 cached input／reasoning output，并验证 provider total。
- provider 已返回 response 但 `choices` 为空或 content 缺失时，adapter 仍返回 metadata；Runtime 先提交 `COMPLETED` attempt，再按既有 invalid reply／pause 语义处理内容。
- `SessionState.llm_attempts` 是跨回合 lifetime ledger；`model_calls` 仍只表达当前用户回合的调用次数，不作为 token 统计来源。
- `/usage` 通过公开 Application query 展示 context safety、lifetime totals、Agent／purpose groups 与最近 attempts；snapshot schema v3 顶层 `llm_attempts` 负责 save／dump／restore。
- 首版仅记录交互式 Main／Resume Runtime；MemoryExtractor、Web Search、embedding、STT／TTS 继续排除。缓存统计只包含 provider 报告的 cached input；output 使用统一价格，不存在 cached output 字段。
- provider failure、timeout 或 cancellation 没有可靠 usage 时保存 typed unavailable，不以 `0` 冒充未知；费用按 uncached input、cached input 与 output 三部分估算。
- 本机 `.env` 的每百万 tokens CNY 参考价已配置为 FLASH `1`／`0.02`／`2`、PRO `3`／`0.025`／`6`（顺序为 uncached input／cached input／output）；该部署配置不纳入 Git。
...
- Settings 新增全局 `LLM_USABLE_CONTEXT_TOKENS`（正整数，缺失时默认 `230000`）与 `LLM_CONTEXT_COMPRESSION_THRESHOLD_RATIO`（`0 < value < 1`，缺失时默认 `0.95`）；`.env.example` 使用 `230000`／`0.95` 作为可见示例。context 配置由 PRO／FLASH 共用，不表示 provider 物理上限，也不校验实际 model mapping；配置者负责保证其安全性。
...
| `application/settings.py` | `Settings` 末尾新增 `llm_usable_context_tokens: int = 230000`、`llm_context_compression_threshold_ratio: float = 0.95`、`llm_pricing: LLMPricing | None = None`，避免扩大到既有直接构造 Settings 的白名单外测试；`from_env()` 对缺失 context key 使用上述默认值，显式值仍严格校验，费用组可选。环境键固定为 `LLM_USABLE_CONTEXT_TOKENS`、`LLM_CONTEXT_COMPRESSION_THRESHOLD_RATIO`、`LLM_COST_UNIT`、`LLM_PRO_INPUT_PRICE_PER_MILLION`、`LLM_PRO_CACHED_INPUT_PRICE_PER_MILLION`、`LLM_PRO_OUTPUT_PRICE_PER_MILLION`、`LLM_FLASH_INPUT_PRICE_PER_MILLION`、`LLM_FLASH_CACHED_INPUT_PRICE_PER_MILLION`、`LLM_FLASH_OUTPUT_PRICE_PER_MILLION` |
...
当前没有“技术上无法实现”的硬阻塞；B-01、B01、B02 已验证关闭，B00 工程实现已完成并通过自动化验证，真实 provider／估算校准仍待用户执行。Interviewer production／测试实现仍未授权。

用户先按 B00 清单执行真实 provider／估算校准并完成剩余验收；B00 关闭后再恢复 **B03 —— typed executor protocol 与 agent-local state 形状**。B00 实施不得扩展进入 Interviewer production code。
