    ...
    def render_event(self, event: RuntimeEvent) -> None:
        if isinstance(event, Progress):
            ...
            if event.plan is not None:
                ...
                self._console.print(Panel(self._preview(event.output), title=f"工具结果 · {event.tool_name}", border_style="dim"))
    ...
    def _render_plan(self, plan: Plan) -> None:
        ...
        for index, item in enumerate(plan.items, start=1):
            ...
            if item.status is PlanStatus.IN_PROGRESS:
                description = f"[bold]{description}[/]"
            table.add_row(str(index), description, labels[item.status])
        self._console.print(table)
